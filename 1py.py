...
def create_resume():
    ...
    story.append(Paragraph("<b>ЖЕЛАЕМАЯ ДОЛЖНОСТЬ:</b> Младший ETL-разработчик / Начинающий Дата-инженер (Junior Data Engineer) / Аналитик данных / Операционный аналитик", subtitle_style))
    
    contact_data = [
        [Paragraph("<b>Номер телефона/telegram:</b> [Введи данные]", body_style), 
         Paragraph("<b>GitHub:</b> https://github.com", body_style)],
        [Paragraph("<b>Почта:</b> [Введи данные]", body_style), 
    ...
    story.append(t_exp1)
    story.append(Paragraph("<i>Позиция: Инженер ПЭО (аналитический отдел) / Инженер ПТО II категории</i>", body_style))
    ...
    story.append(t_exp2)
    story.append(Paragraph("<i>Позиция: Инженер ПТО II категории</i>", body_style))
    story.append(Spacer(1, 3))
    ...
    story.append(Paragraph("• <b>Глубокий рефакторинг систем учета:</b> Самостоятельно провел полный audit унаследованных (Legacy) учетных таблиц Excel. Обнаружил и ликвидировал скрытые формульные ошибки, оптимизировал интерфейс ввода данных для полевых сотрудников, что кардинально сократило трудозатраты на ежедневную рутинную проверку.", body_style))
    story.append(Spacer(1, 6))
