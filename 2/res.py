...
def create_resume(output_filename="resume.pdf"):
    ...
    story.append(Paragraph("<b>Целевая должность:</b> Младший ETL-разработчик / Начинающий Дата-инженер", subtitle_style))
    story.append(Paragraph("<b>Локация:</b> Саратов, Россия (Удаленная работа / Гибрид / Релокация)", contacts_style))
    story.append(Paragraph("<b>Контакты:</b> [Номер телефона / Telegram] | [Почта] | [GitHub]", contacts_style))
    ...
    story.append(Paragraph("<b>Базы данных и SQL:</b> PostgreSQL, DBeaver, DDL, оконные функции, aggregate-функции, оптимизация запросов, представления (Views).", body_style))
    ...
    story.append(Paragraph("<b>Дополнительно:</b> Понимание концепций ETL/ELT, проектирование реляционных структур данных, валидация и очистка данных (Data Quality).", body_style))
    
    # Опыт разработки и пет-проекты
    add_section_divider("Опыт разработки и пет-проекты")
    story.append(Paragraph("<b>Симуляция локального Финтех ELT-пайплайна (Август 2026 г.)</b>", job_title_style))
    ...
    story.append(Paragraph("• Спроектировал DDL-структуру таблиц-приемников в PostgreSQL. Написал SQL-скрипты очистки, приведения форматов и агрегации данных в витринах (CREATE VIEW).", bullet_style))
    ...
    story.append(Paragraph("• Реализовал и протестировал чистые SQL-запросы для 33 практических бизнес-сценариев (анализ метрик, сегментация, агрегация данных).", bullet_style))
    ...
    story.append(Paragraph("• <b>Data Quality:</b> Проводил аудит входящей отчетности, выявлял логические ошибки, расхождения и системные коллизии в первичных данных.", bullet_style))
    ...
    story.append(Paragraph("• <b>Оптимизация:</b> Провел аудит унаследованных учетных таблиц Excel, ликвидировал формульные ошибки и оптимизировал интерфейс ввода данных. Переведен на инженерную должность за автоматизацию рутинных процессов.", bullet_style))
    ...
    story.append(Paragraph("• <b>Анализ данных:</b> Проводил аудит неструктурированных запросов, сопоставлял потребности клиентов с техническими параметрами базы данных в рамках SLA.", bullet_style))

    # Образование и сертификаты
    add_section_divider("Образование и сертификаты")
    story.append(Paragraph("• <b>ППК СГТУ им. Гагарина Ю.А. (2017 — 2021 гг.)</b> | Специальность: «Компьютерные сети», Диплом с отличием.", bullet_style))
